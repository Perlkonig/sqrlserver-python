...
def test_nutgen():
	...
	#unsecured
	nut = nut.generate(nonce, '0.0.0.0', counter)
	...
	#real ipv4
	nut = nut.generate(nonce, '155.6.0.126', counter)
	nutqr = nut.toString('qr')
	nutlink = nut.toString('link')
	assert nutqr != nutlink
	nuts['ipv4-qr'] = nutqr
	...
	#real ipv6
	nut = nut.generate(nonce, '2001:db8:a0b:12f0::1', counter)
	nutqr = nut.toString('qr')
	nutlink = nut.toString('link')
	assert nutqr != nutlink
	nuts['ipv6-qr'] = nutqr
...
def test_nutval():
	...
	nut = sqrlserver.Nut(key)
	nut = nut.load(nonce, nuts['unsecured-qr']).validate('0.0.0.0', 600, counter)
	assert nut.ipmatch
	assert nut.fresh
	...
	nut = nut.load(nonce, nuts['unsecured-link']).validate('0.0.0.0', 600, counter)
	assert nut.ipmatch
	assert nut.fresh
	assert nut.countersane
	assert nut.islink
	...
	nut = nut.load(nonce, nuts['ipv4-qr']).validate('155.6.0.126', 600, counter)
	assert nut.ipmatch
	assert nut.fresh
	assert nut.countersane
	assert nut.isqr
	...
	nut = nut.load(nonce, nuts['ipv4-link']).validate('155.6.0.126', 600, counter)
	assert nut.ipmatch
	assert nut.fresh
	assert nut.countersane
	assert nut.islink
	...
	nut = nut.load(nonce, nuts['ipv6-qr']).validate('2001:db8:a0b:12f0::1', 600, counter)
	assert nut.ipmatch
	assert nut.fresh
	assert nut.countersane
	assert nut.isqr
	...
	nut = nut.load(nonce, nuts['ipv6-link']).validate('2001:db8:a0b:12f0::1', 600, counter)
	assert nut.ipmatch
	assert nut.fresh
	assert nut.countersane
	assert nut.islink
	...
	nut = nut.load(nonce, nuts['ipv4-link']).validate('155.6.0.125', 600, counter)
	assert not nut.ipmatch
	nut = nut.load(nonce, nuts['ipv6-link']).validate('2002:db8:a0b:12f0::1', 600, counter)
	assert not nut.ipmatch

	#too old
	t = time.time() - 3000
	nut = nut.generate(nonce, '155.6.0.126', counter, t).validate('155.6.0.126', 600, counter)
	...
	#counter too small
	nut = nut.generate(nonce, '155.6.0.126', 5).validate('155.6.0.126', 600, counter, counter-10)
	assert not nut.countersane

	#counter too big
	nut = nut.generate(nonce, '155.6.0.126', counter + 100).validate('155.6.0.126', 600, counter)
	assert not nut.countersane
...
def test_urlgen():
	nut = sqrlserver.Nut(key).generate(nonce, '155.6.0.126', counter).toString('qr')
...
def test_request():
	...
	client = sqrlserver._extract_client(clientstr)
	...
	server = sqrlserver._extract_server(serverstr)
